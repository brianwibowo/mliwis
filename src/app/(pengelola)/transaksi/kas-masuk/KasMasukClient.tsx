...
import { createKasMasuk, updateKasMasuk, deleteKasMasuk } from '../actions'
import { formatRupiah, formatTanggal } from '@/lib/format'
import { JENIS_KAS_MASUK } from '@/lib/constants'
...
export default function KasMasukClient({ initialData }: Props) {
  ...
  const navigate = (m: number, y: number, p?: number) => {
    router.push(`/transaksi/kas-masuk?month=${m}&year=${y}${p && p > 1 ? `&page=${p}` : ''}`)
  }

  const handleSubmit = (formData: FormData) => {
    startTransition(async () => {
      const r = editData ? await updateKasMasuk(editData.id, formData) : await createKasMasuk(formData)
  ...
  const handleDelete = () => {
    if (!deleteId) return
    startTransition(async () => {
      const r = await deleteKasMasuk(deleteId)
      if (r.error) addToast(r.error, 'error')
  ...
  return (
    <div>
      <div className="page-header">
        <div className="page-header-left"><h1>Kas Masuk</h1><p>Pemasukan Pantai Mliwis</p></div>
        <button className="btn btn-primary" onClick={() => { setEditData(null); setSelectedJenis(''); setShowModal(true) }}><Plus size={18} /> Tambah Kas Masuk</button>
      </div>
      ...
      <div className="stat-card mb-6">
        ...
        <div className="stat-content">
          ...
          <div className="stat-label">Total Kas Masuk — {BULAN[month - 1]} {year}</div>
        </div>
      ...
      <div className="card">
            ...
            <tbody>
              ...
              {data.map((k, i) => (
                <tr key={k.id}>
                  ...
                  <td className="font-semibold text-success">{formatRupiah(k.nominal)}</td>
        ...
        {totalPages > 1 && (
          <div className="card-footer"><div className="pagination">
            {Array.from({ length: totalPages }, (_, i) => (<button key={i} className={`pagination-btn ${currentPage === i + 1 ? 'active' : ''}`} onClick={() => navigate(month, year, i + 1)}>{i + 1}</button>))}
          </div></div>
        )}
      </div>

      <Modal isOpen={showModal} onClose={() => { setShowModal(false); setEditData(null) }} title={editData ? 'Edit Kas Masuk' : 'Tambah Kas Masuk'}>
        <form action={handleSubmit}>
          <div className="form-row">
            ...
            <div className="form-group"><label className="form-label">Nominal (Rp) <span className="required">*</span></label><input name="nominal" type="number" className="form-input" defaultValue={editData?.nominal} placeholder="0" required /></div>
          </div>
          <div className="form-group">
            ...
            <select name="jenisTransaksi" className="form-select" value={selectedJenis} onChange={(e) => setSelectedJenis(e.target.value)} required>
              ...
              {JENIS_KAS_MASUK.map((j) => <option key={j} value={j}>{j}</option>)}
            </select>
          </div>
          {selectedJenis === 'Pendapatan Lain-lain' && (
            <div className="form-group"><label className="form-label">Keterangan Lain</label><input name="keteranganLain" className="form-input" defaultValue={editData?.keteranganLain || ''} placeholder="Jenis pendapatan lain..." /></div>
          )}
          <div className="form-group"><label className="form-label">Keterangan</label><textarea name="keterangan" className="form-textarea" defaultValue={editData?.keterangan || ''} placeholder="Keterangan tambahan..." /></div>
      ...
      <Modal isOpen={deleteId !== null} onClose={() => setDeleteId(null)} title="Hapus Data" size="sm">
        <p>Yakin hapus data kas masuk ini?</p>
